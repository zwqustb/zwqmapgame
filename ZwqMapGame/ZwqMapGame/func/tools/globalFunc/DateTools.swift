...
class DateTools: NSObject {
    ...
    //时分字符串转化成分钟
    class func convertHM2M(_ str1:String)->Int{
        let aryHM = str1.components(separatedBy: ":")
        let nH = Int(aryHM.first ?? "0") ?? 0
        let nM = Int(aryHM.last ?? "0") ?? 0
        return nH*60+nM
    }
    //获取多少小时后的HM
    class func addHour(_ nH:Int ,ForHM strHM:String)->String{
        let date1 = DateTools.string2Date(strHM, "HH:mm")
        let date2 = date1.addingTimeInterval(TimeInterval(nH*60*60))
        return date2.toString("HH:mm")
    }
}
