...
//    }
        ...
        if m_numRange != nil {
            ...
            if maxStr != nil{
                ...
                if maxNum != nil && curValue != nil && curValue! > maxNum! {
                    BOEProgressHUD.showError(withStatus: "输入值不能超出最大值\(maxNum!)")
                    return false
                }
            }
            ...
            if aryCur.count == 2{
                ...
                if nCurDecimalCount > nDecimal{
                    if nDecimal  == 0{
                        BOEProgressHUD.showError(withStatus: "不能为小数")
                    }else{
                        BOEProgressHUD.showError(withStatus: "小数位数不能超过\(nDecimal)位")
                    }
                    return false
                }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        //lanwenjin-------结束输入时的判断最小值
        if m_numRange != nil {
            let arySubString = m_numRange!.split(separator: "~")
            if arySubString.count == 2{
                let curValue = Double(textField.text ?? "0")
                let minStr = arySubString.first
                if minStr != nil{
                    let minNum = Double(String(minStr!))
                    if minNum != nil && curValue != nil && curValue! < minNum! {
                        BOEProgressHUD.showError(withStatus: "输入值不能小于最小值\(minNum!)")
                        textField.text = nil
                    }
                }
            }
