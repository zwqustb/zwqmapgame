...
// 传入    不加st、ut、的url
    ...
    //webview 的实现  监听webview 的跳转 如果是“logout.html”则跳转登录界面
    ...
    func configBoeSafeInfo() {
        ...
        //lanwenjin---添加通知授权信息
        let jsString3 = "localStorage.setItem('NotificationAuthority', '\(1)')"
        self.evaluateJavaScript(jsString3) { (pAny, pError) in
        }
//        BOELocalNotificationTool.canReciveNotification { (isOn) in
//            DispatchQueue.main.async {
//                let jsString3 = "localStorage.setItem('NotificationAuthority', '\(isOn)')"
//                self.evaluateJavaScript(jsString3) { (pAny, pError) in
//                }
//            }
//        }
