...
class WebVC: BaseViewController {
    ...
    //网络分享的地址 或者是 图片分享的类名
    var shareTip:String?
    @objc class func showWebWith(urlString:String,_ title:String,_ strSysName:String = ""){
        let pWeb = WebVC.init()
        pWeb.pUrl = URL.init(string: urlString)
        pWeb.strTitle = title
         pWeb.strSysName = strSysName
        pushView(pWeb)
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let strTitle = strTitle{
            if strTitle == "SQI解释"{//睡眠仪
                let buryingPointPageName = "页面_设备_睡眠仪_SQI解释"
                BOEBuryingPointTool.pageViewEvent(buryingPointPageName as NSString)
            }
        }
        if shareTip == "BodyFatShareView" {
            BOEBuryingPointTool.pageViewEvent("页面_设备_体脂称_趋势报告")
        }
        
        if strTitle == "积分兑换记录" {
            BOEBuryingPointTool.pageViewEvent("页面_我的积分_兑换记录")
        }
        
        if strTitle == "兑换规则" {
            BOEBuryingPointTool.pageViewEvent("页面_我的积分_兑换规则")
        }
        
        if strTitle == "历史记录" {
            BOEBuryingPointTool.pageViewEvent("页面_尿检仪_数据记录")
        }
        
        if strTitle == "测量说明" {
            BOEBuryingPointTool.pageViewEvent("页面_尿检仪_测量说明")
        }

        if strTitle == "DGB帮助" {
            BOEBuryingPointTool.pageViewEvent("页面_DBG_帮助")
        }
        if strSysName != "" {
            BOEBuryingPointTool.pageViewEvent(strSysName as NSString)
        }
    }
...
//        self.navigationController?.navigationBar.titleTextAttributes = [NSForegroundColorAttributeName:UIColor.appBlackColor()]
    ...
    @objc func clickShare(){
        BOEBuryingPointTool.moduleInnerCommonButtonClick(["page_name": title ?? "趋势图","module_name": shareTip == "BodyFatShareView" ? "体脂秤":" ","button_name":"分享"])
        switch m_nShareType {
        case 1:
            break
        case 2:
